...
export function useUpload(
  ...
  const fileProcessor = useMemo(
    () => new FileProcessor(options),
    [options.maxFiles, options.maxFileSize, options.allowedTypes]
  );
  ...
  useEffect(() => {
    const initProvider = async () => {
      try {
        ...
        setError(
          `Failed to initialize ${provider.getName()} provider: ${error instanceof Error ? error.message : "Unknown error"}`
        );
      }
    };
  ...
  const uploadFile = useCallback(
    async (fileId: string, uploadOptions?: UploadOptions) => {
  ...
  const uploadedSize = useMemo(
    () => successFiles.reduce((sum, f) => sum + f.size, 0),
    [successFiles]
  );
